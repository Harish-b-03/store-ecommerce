...
export const getServerSideProps: GetServerSideProps = async (context) => {
	...
	const res_electronic = await fetch(
		`${GET_BY_CATEGORY}/electronics?limit=4`
	);
	const electronic: any = await res_electronic.json();

	const res_womensDresses = await fetch(
		`${GET_BY_CATEGORY}/women's clothing?limit=4`
	);
	const womensDresses: any = await res_womensDresses.json();

	const res_mensDresses = await fetch(
		`${GET_BY_CATEGORY}/men's clothing?limit=4`
	);
